...
void save_pic(char *filename, char *datPath, char *palPath, int width, int height, int Link[])
{
	...
	int total_tile = width*height;
	int rWidth = width*SINGLE_SIZE, rHeight = height*SINGLE_SIZE;
...
typedef unsigned short WORD;
	...
	{
		now = realBMP+(Link[i]/width)*(rWidth>>1)*SINGLE_SIZE+(Link[i]%width*SINGLE_SIZE>>1);
...
int main(int argc, char *argv[])
{
	...
	//	return 1;
//	int list[24] = {0,1,2,3,6,7,8,9,12,13,14,15,18,19,20,21,4,5,10,11,16,17,22,23};
    int list[8]={0,1,2,3,4,5,6,7};
	save_pic(argv[1],argv[2],"3.pal",4,2,list);
	return 0;
}
